...
def parse_mentor_topics(mentor_row, topic_map):
    codes = set()
    # pick columns that contain 'topic' in header
...
def clean_mentees(mentee_csv, valid_codes):
    ...
    with open(mentee_csv, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        # find preference columns (start with 'PREFERENCE')
        ...
        for row in reader:
            ...
            for c in pref_cols[:5]:
                ...
                val = val.upper()
                if val in ('', 'N/A', 'NA', 'NONE', 'NULL', '-', 'NULL'):
                    val = ''
...
def allocate(cleaned_mentees, topic_capacity):
    allocations = []
    # process in the order present in cleaned_mentees (file order)
...
def write_allocations(out_path, allocations):
    with open(out_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Timestamp','Full Name','Assigned Code','Preference Rank','Pref1','Pref2','Pref3','Pref4','Pref5'])
